...
final class SitemapCanonicalXmlWriter
{
    ...
    /**
     * @param list<SharedSitemapIndexEntryDTO> $entries
     */
    public function renderSharedIndex(array $entries): string
    {
        $normalized = [];
        foreach ($entries as $entry) {
    ...
    private function normalizeIndexEntry(mixed $sitemap): array
    {
        ...
        return [
            'loc' => $normalizedLoc,
            'lastmod' => $this->nullableIndexArrayString($sitemap, 'lastmod'),
        ];
    }
     ...
     * @return array{loc: string, lastmod: ?string}
     */
    private function normalizeSharedIndexEntry(SharedSitemapIndexEntryDTO $entry): array
    {
        return $this->normalizeIndexValues($entry->loc, $entry->lastmod);
    }

    /**
     * @return array{loc: string, lastmod: ?string}
     */
    private function normalizeIndexValues(string $loc, ?string $lastmod): array
    {
        return [
            'loc' => trim($loc),
            'lastmod' => $lastmod === null ? null : trim($lastmod),
        ];
    }
     ...
     */
    private function nullableIndexArrayString(array $sitemap, string $field): ?string
    {
        ...
        if (!SitemapUrlDTO::isValidLastmod($value)) {
            throw SeoInvalidArgumentException::emptyField($field);
        }
     ...
     * @param array{loc: string, lastmod: ?string} $entry
     */
    private function writeIndexEntry(XMLWriter $writer, array $entry): void
    {
