...
The Maatify SEO library provides three high-level preset factories in `src/Web/Page/` tailored for specific business domains. These factories abstract away the lower-level builder and schema construction (as implemented in `SeoPagePresetFactory` and the JSON-LD builders) to provide simple, purpose-driven methods for generating complete SEO metadata sets for common page types.
