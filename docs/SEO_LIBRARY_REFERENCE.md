...
Phase 24's normative output rules are recorded in the
[MetaGeneratorService contract](SEO/library/META_GENERATOR_SERVICE_CONTRACT.md).
Defaults use PHP `trim()` semantics; overrides are independent by field, and a
blank override leaves its default intact. `SeoNotFoundException` means no override,
while other exceptions propagate. Canonical precedence is explicit value, then
host-generated value, then `null`; an explicit canonical prevents a host call, and
a host URL is returned unchanged.

The final title, description, and canonical are copied to their corresponding
Open Graph and Twitter fields. `openGraphType`, `openGraphImage`, `twitterCard`,
and `twitterImage` are not generated by this service. The contract adds no
URL/provider validation or network behavior, and `MetaTagsDTO`'s public shape is
unchanged.
