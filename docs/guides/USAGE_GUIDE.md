...
## 16. Optional Merchant Center Diagnostics

The optional Merchant Center boundary allows fetching product eligibility via Google Merchant API v1. The library does not perform HTTP or handle OAuth credentials. The host application must supply a transport implementation.
...
final readonly class HostMerchantCenterTransport implements MerchantCenterTransportInterface
{
    ...
    {
        $response = $this->gateway->getProduct($request->name);
    ...
    // Host implements listAggregateProductStatuses as well...
}

$service = new MerchantCenterDiagnosticsService(
    new HostMerchantCenterTransport($hostGateway),
    new MerchantCenterResponseMapper(),
);

$result = $service->getProduct(new MerchantCenterProductRequestDTO('accounts/123/products/abc'));
```
