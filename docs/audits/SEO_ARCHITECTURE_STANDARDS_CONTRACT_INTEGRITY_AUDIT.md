...
### Exact Sitemap `loc` URL/URI/IRI lexical predicate (closed by the contract completeness sweep)

For this remediation the Sitemap `loc` lexical acceptance/rejection set is fixed exactly and must **not** be re-derived through `FILTER_VALIDATE_URL` or any implementation-specific parser choice. The predicate (FIX 2) is:

- **Missing:** `loc === null || trim(loc) === ''` → `sitemap_loc_missing`; a missing `loc` does **not** enter URL lexical validation.
- **Non-empty candidate:** the validator checks the value **as supplied** (no trim/normalization before the lexical decision, except `trim()` used only to detect missing/whitespace-only); a non-empty value remains the original authored value.
- A non-empty `loc` is lexically accepted only when it satisfies **all** of: (1) absolute scheme matching `[A-Za-z][A-Za-z0-9+.-]*`; (2) followed immediately by `://`; (3) non-empty authority/host after `://`; (4) no ASCII control characters `U+0000..U+001F`; (5) no `U+007F`; (6) no raw space `U+0020`; (7) no malformed percent escape — every `%` followed by exactly two ASCII hex digits (`0-9 A-F a-f`); (8) raw Unicode allowed in the candidate IRI; (9) no DNS/IDNA/network resolution; (10) the generic Sitemap protocol profile does **not** require HTTP/HTTPS only — scheme must only satisfy the absolute-scheme grammar, host/location scope semantics stay under `sitemap_location_scope_violation`; (11) a `#...` fragment is **not acceptable** in a Sitemap `loc`; (12) no malformed authority (host after `://` must be present). Any violated point → `sitemap_loc_invalid_uri_iri`.
- The same predicate applies to the Sitemap Index child `loc` with the same missing-vs-invalid precedence and target `sitemap_index_entry` + child index + `field = sitemap`.
- This predicate is locked in the GDC-01 machine table and Stack 4; it is **not** an implementation choice.
...
### Exact Video URL lexical predicate (closed by the contract completeness sweep)

The URL-shape rules (`google_video_thumbnail_loc_invalid_url`, `google_video_content_loc_invalid_url`, `google_video_player_loc_invalid_url`) do not rely on a vague "valid URL shape" and are not an implementation choice. The exact predicate (FIX 4), shared by all three diagnostics and locked in GDC-01 / Stack 4:
...
- A present value is URL-shaped (lexically accepted) when it satisfies **all** of: (1) scheme matching `[A-Za-z][A-Za-z0-9+.-]*`; (2) followed immediately by `://`; (3) non-empty authority/host after `://`; (4) no ASCII control characters `U+0000..U+001F`; (5) no `U+007F`; (6) no raw space `U+0020`; (7) every `%` followed by exactly two ASCII hex digits (`0-9 A-F a-f`); (8) raw Unicode allowed (ASCII-only `FILTER_VALIDATE_URL` is never the contract); (9) no network/DNS/remote inspection; (10) a `#...` fragment is allowed lexically and is not a diagnostic cause; (11) scheme-specific provider support is not part of these URL-shape diagnostics.
- **Data URL anti-cascade (fixed):** a candidate whose value starts the semantic form `data:` is a special provider-disallowed scheme and emits **only** `google_video_data_url_unsupported` for the relevant field; the absence of `://` in that value never also yields the corresponding `*_invalid_url`. No `unsupported video scheme` diagnostic is introduced.
- `FILTER_VALIDATE_URL` and any parser/helper are implementation mechanisms only; the observable acceptance/rejection set must match this predicate literally.
...
| `sitemap_loc_missing` | error | protocol | `sitemaps` | `loc` (URL) / `sitemap` (Index child) | null | Entry-level missing-`loc` rule: emit only when `loc === null || trim(loc) === ''`. A missing `loc` does **not** enter URL lexical validation. For a URL Sitemap entry: `field = loc`, target `sitemap_url` + URL entry index. For a Sitemap Index entry: `field = sitemap`, target `sitemap_index_entry` + child index. Missing `loc` emits this diagnostic only and never also `sitemap_loc_invalid_uri_iri`. Companion-only, not legacy result. |
| `sitemap_loc_invalid_uri_iri` | error | protocol | `sitemaps` | `loc` (URL) / `sitemap` (Index child) | null | A non-empty supplied `loc` fails the **exact fixed Sitemap URL/URI/IRI lexical predicate** locked under this table (F-02, Stack 4; the predicate is not an implementation choice). Missing/whitespace-only uses `sitemap_loc_missing`. Lexical predicate (applied to the value **as supplied**, no normalization before the decision): (1) absolute scheme matching `[A-Za-z][A-Za-z0-9+.-]*`; (2) followed immediately by `://`; (3) non-empty authority/host after `://`; (4) no ASCII control characters `U+0000..U+001F`; (5) no `U+007F`; (6) no raw space `U+0020`; (7) no malformed percent escape — every `%` followed by exactly two ASCII hex digits (`0-9 A-F a-f`); (8) raw Unicode outside ASCII is allowed in the candidate IRI; (9) no DNS/IDNA/network resolution; (10) the generic Sitemap protocol profile does **not** require HTTP/HTTPS only — host/location scope semantics are separate under `sitemap_location_scope_violation`; (11) a `#...` fragment is **not acceptable** in a Sitemap `loc`; (12) no malformed authority (for example no host after `://`). Any violated point → emit. Same field/target mapping as `sitemap_loc_missing` (URL: `field = loc`, `sitemap_url`; Index child: `field = sitemap`, `sitemap_index_entry`). `FILTER_VALIDATE_URL` and any parser/helper are **not** the lexical contract; only this observable predicate is. XML escaping failure is **not** part of this diagnostic. Companion-only, not legacy result. |
...
**Exact Sitemap URL/URI/IRI lexical predicate (fixed).** The trigger for `sitemap_loc_invalid_uri_iri` is the **exact** predicate below. `FILTER_VALIDATE_URL` and any implementation-specific parser/validator/regex/helper are **not** the lexical contract for this validation surface; they may only be used as implementation mechanisms whose observable acceptance/rejection set must match this predicate literally.

- **Missing:** `loc === null || trim(loc) === ''` → `sitemap_loc_missing`, and **does not** enter URL lexical validation.
- **Non-empty candidate normalization boundary:** the validator checks the value **as supplied**; no trim/normalization precedes the lexical decision, other than using `trim()` **only** to detect missing/whitespace-only. A non-empty value remains the original authored value.

A non-empty Sitemap `loc` is lexical-accepted only when it satisfies **all** of:

1. contains an absolute scheme matching `[A-Za-z][A-Za-z0-9+.-]*`;
2. followed immediately by `://`;
3. a non-empty authority/host after `://`;
4. no ASCII control characters `U+0000..U+001F`;
5. no `U+007F`;
6. no raw space `U+0020`;
7. no malformed percent escape — every `%` is followed by exactly two ASCII hexadecimal digits (`%HH`, H in `0-9 A-F a-f`);
8. raw Unicode characters outside ASCII are allowed in the candidate IRI (a Unicode hostname/path alone never causes rejection);
9. no DNS/IDNA/network resolution;
10. the generic Sitemap protocol profile does **not** require HTTP/HTTPS only — the scheme must only satisfy the absolute-scheme grammar; host/location scope semantics are separate and remain under `sitemap_location_scope_violation`;
11. a `#...` fragment is **not acceptable** in a Sitemap `loc` for this library contract;
12. no malformed authority — the host after `://` must be present.

If a non-empty candidate violates any point above → `sitemap_loc_invalid_uri_iri`. This is the complete predicate (FIX 2).

**Sitemap Index child `loc` (FIX 3).** The exact same predicate applies to `SitemapIndexEntryValidationInputDTO::$loc` with the same missing-vs-invalid precedence: missing → `sitemap_loc_missing` only; non-empty violating any point above → `sitemap_loc_invalid_uri_iri` only. Targets stay `scope = sitemap_index_entry`, `entryIndex = child index`, `field = sitemap`. There is **no** different lexical contract for the Index child URL.
...
| `google_video_thumbnail_loc_invalid_url` | warning | provider | `google` | `thumbnail_loc` | — | `thumbnailLoc` is non-empty (`!== null && trim(value) !== ''`) but fails the **exact fixed Google Video URL lexical predicate** locked under this table (F-04; shared by all three `*_invalid_url` diagnostics). Missing/whitespace-only uses `google_video_thumbnail_loc_missing` instead; the two never emit together for the same item. Target `sitemap_video` + `[urlIndex][videoIndex]`. |
| `google_video_content_loc_invalid_url` | warning | provider | `google` | `content_loc` | — | `contentLoc` is present/non-empty but fails the **exact fixed Google Video URL lexical predicate** locked under this table. Target `sitemap_video` + `[urlIndex][videoIndex]`. |
| `google_video_player_loc_invalid_url` | warning | provider | `google` | `player_loc` | — | `playerLoc` is present/non-empty but fails the **exact fixed Google Video URL lexical predicate** locked under this table. Target `sitemap_video` + `[urlIndex][videoIndex]`. |
...
**Exact Google Video URL lexical predicate (fixed, FIX 4).** The three diagnostics `google_video_thumbnail_loc_invalid_url`, `google_video_content_loc_invalid_url`, and `google_video_player_loc_invalid_url` share **one** predicate. `FILTER_VALIDATE_URL` and any implementation-specific parser/validator/regex/helper are **not** the lexical contract; they may only be used as implementation mechanisms whose observable acceptance/rejection set matches this predicate literally.
...
- **Accepted lexical structure:** a present value is URL-shaped (valid at the lexical level for this remediation) when it satisfies **all** of:
  1. a scheme matching `[A-Za-z][A-Za-z0-9+.-]*`;
  2. followed immediately by `://`;
  3. a non-empty authority/host after `://`;
  4. no ASCII control characters `U+0000..U+001F`;
  5. no `U+007F`;
  6. no raw space `U+0020`;
  7. every `%` followed by exactly two ASCII hex digits (`0-9 A-F a-f`);
  8. raw Unicode is allowed — an ASCII-only `FILTER_VALIDATE_URL` rule is never the contract;
  9. no network / DNS / remote inspection;
  10. a `#...` fragment is allowed lexically and is **not** a diagnostic cause in this remediation;
  11. scheme-specific provider support is **not** part of these URL-shape diagnostics, except the independent Data URL contract.

- A malformed absolute URL structure → the corresponding `*_invalid_url` diagnostic.
- **Data URL anti-cascade (explicit):** a candidate whose value starts the semantic Data URL form `data:` is treated as a special provider-disallowed scheme. The absence of `://` in that value must **not** additionally produce the corresponding `*_invalid_url`; the value emits **only** `google_video_data_url_unsupported` for the relevant field under the existing independent Data URL contract.
...
| `hreflang_url_not_fully_qualified` | warning | provider | `google` | `href` | null | Alternate URL not fully qualified where the Google profile requires it (F-15). This single code covers **missing and non-fully-qualified** candidate URLs for `HreflangValidationLinkDTO::$url`; the trigger is the **exact fixed fully-qualified lexical predicate** locked under this table (FIX 5). No separate missing/invalid/relative URL code is introduced and no duplicate URL diagnostic is emitted for one link. Target `hreflang_link` + page index + link index. Companion-only, not legacy result. |
...
**Exact Hreflang URL fully-qualified lexical predicate (fixed, FIX 5).** For `hreflang_url_not_fully_qualified`, the trigger is the **exact** predicate below; `FILTER_VALIDATE_URL` and any implementation-specific parser/validator/regex/helper are **not** the lexical contract for this validation surface and may only be used as implementation mechanisms whose observable acceptance/rejection set matches this predicate literally.
...
or when a non-empty value fails **any** of the following (all must hold):
...
6. no raw space `U+0020`;
7. every `%` followed by exactly two ASCII hex digits (`0-9 A-F a-f`);
8. raw Unicode is allowed (Unicode hostnames/paths are not rejected);
9. no network/DNS validation — the predicate is purely lexical;
10. a `#...` fragment is **not** a rejection cause in the hreflang URL contract for this remediation.
...
**Fully-qualified alternate URL predicate (`hreflang_url_not_fully_qualified`).** Emit when `url === null || trim(url) === ''`, or when a non-empty value fails **any** of: (1) scheme matching `[A-Za-z][A-Za-z0-9+.-]*`; (2) followed immediately by `://`; (3) non-empty authority/host after `://`; (4) no ASCII control characters `U+0000..U+001F`; (5) no `U+007F`; (6) no raw space `U+0020`; (7) every `%` followed by exactly two ASCII hex digits (`0-9 A-F a-f`); (8) raw Unicode allowed; (9) no network/DNS validation; (10) a `#...` fragment is not a rejection cause. Exactly one diagnostic per failed link. `FILTER_VALIDATE_URL` is not the contract; no membership/provider-host inference.
...
**Lexical predicates are locked contracts (addendum, FIX 10).** Whenever a companion diagnostic's trigger depends on URL/IRI/hreflang lexical validity, the exact lexical predicate (Sitemap `loc`, Google Video URL-shape, hreflang fully-qualified URL, hreflang tag-syntax) fixed by this Audit is part of the machine contract and is **not** an implementation choice. Implementation libraries, PHP built-ins, regex helpers, URI parsers, and internal utilities may only be used as implementation mechanisms; they must not redefine the observable acceptance/rejection set. `FILTER_VALIDATE_URL` is not an authoritative lexical contract for any of those validation surfaces.
...
- malformed non-empty `loc`: a non-empty supplied `loc` that fails the **exact fixed Sitemap URL/URI/IRI lexical predicate** (FIX 2) emits `sitemap_loc_invalid_uri_iri` (error; same field/target mapping as `sitemap_loc_missing`). The predicate is applied to the value **as supplied** and is the full 12-point lexical contract locked in F-02/GDC-01; `FILTER_VALIDATE_URL` and any parser/helper are implementation mechanisms only, never the contract. The same predicate applies to the Sitemap Index child `loc` (`field = sitemap`, target `sitemap_index_entry` + child index) with the same missing-vs-invalid precedence. XML escaping failure is **not** part of this diagnostic.
...
- non-empty-but-malformed media/thumbnail URL shapes → `google_video_thumbnail_loc_invalid_url`, `google_video_content_loc_invalid_url`, `google_video_player_loc_invalid_url` (warning, GDC-01 companion-only; target `sitemap_video` + `[urlIndex][videoIndex]`), using the **exact shared Video URL lexical predicate** (FIX 4) applied to non-empty values (`!== null && trim(value) !== ''`); `FILTER_VALIDATE_URL` is not the contract. A missing thumbnail uses `google_video_thumbnail_loc_missing`, never both codes.
...
- Data URLs unsupported → `google_video_data_url_unsupported` (warning, GDC-01 companion-only) with `field = content_loc` when `contentLoc` is a Data URL, `field = player_loc` when `playerLoc` is a Data URL, and **two** same-code diagnostics (content and player) when both locations are Data URLs; no separate codes are created. **Anti-cascade (FIX 4):** a value starting the semantic form `data:` emits only `google_video_data_url_unsupported` for the relevant field and never also the corresponding `*_invalid_url` solely because it lacks `://`.
...
- preserve `x-default`.
- validate deterministically knowable Google structural rules from supplied data. `hreflang_url_not_fully_qualified` (warning, GDC-01 companion-only) is the single code for alternate URL issues: emit when `HreflangValidationLinkDTO::$url` is `null`, `trim(url) === ''`, or fails the **exact fixed fully-qualified lexical predicate** (FIX 5) locked in GDC-01/F-15/Section 9 — the 10-point predicate (absolute scheme `[A-Za-z][A-Za-z0-9+.-]*` immediately followed by `://`, non-empty authority/host, no ASCII control characters, no `U+007F`, no raw space, no malformed percent escape, raw Unicode allowed, no network/DNS, fragment not a rejection cause). `FILTER_VALIDATE_URL` is not the contract. No separate missing/invalid URL code and no duplicate per-link URL diagnostic.
...
- **structural-invalid link exclusion (fixed, FIX 7):** a candidate link is excluded from semantic cluster-edge computation (reciprocity, self-reference matching, alternate-set equality) **if and only if** it produces `hreflang_tag_invalid_syntax` or `hreflang_url_not_fully_qualified` under the exact predicates above; no additional exclusion condition is invented. The excluded link may still produce its own link-level diagnostic; cluster-level diagnostics are computed from the structurally usable links only; no cascade diagnostic is added solely because a structurally invalid link was excluded.
...
- **exact Sitemap URL/URI/IRI lexically-accepted cases (fixed):** `https://example.com/path`, `http://example.com/`, `https://example.com/مسار`, `custom+scheme://example.com/path` each emit **no** `sitemap_loc_invalid_uri_iri` and **no** `sitemap_loc_missing` (non-empty value satisfying every point of the exact fixed predicate).
- **exact Sitemap URL/URI/IRI lexically-rejected cases (fixed):** `example.com/path` (no scheme), `/path` (no scheme), `https:///path` (empty authority), `https://` (empty authority), `https://example.com/a b` (raw space), `https://example.com/%ZZ` (malformed percent escape), `https://example.com/abc%` (malformed percent escape), `https://example.com/path#fragment` (fragment) each emit **only** `sitemap_loc_invalid_uri_iri` (error, `field = loc`, target `sitemap_url` + URL `entryIndex`).
- missing-vs-malformed precedence for the exact predicate: missing → **only** `sitemap_loc_missing`; malformed non-empty → **only** `sitemap_loc_invalid_uri_iri`; no single case emits both.
- the exact-predicate accepted/rejected cases above also apply to the Sitemap Index child `loc` with `field = sitemap`, target `sitemap_index_entry` + child index, and the same missing-vs-invalid precedence.
- **no `FILTER_VALIDATE_URL` behavior may be used as the expected contract** in any Sitemap `loc` lexical test; the expected acceptance/rejection set must be asserted literally from the exact predicate.
...
- **exact Video URL-shape lexically-accepted cases (fixed):** `https://example.com/video.mp4`, `ftp://example.com/video.mp4`, `https://example.com/فيديو.mp4` each emit **no** `*_invalid_url` diagnostic for the populated field (HTTP/HTTPS only is not required; scheme must only satisfy the exact fixed predicate).
- **exact Video URL-shape lexically-rejected cases (fixed):** `/video.mp4` (no scheme), `https:///video.mp4` (empty authority), `https://example.com/a b.mp4` (raw space), `https://example.com/%ZZ` (malformed percent escape) each emit the corresponding `*_invalid_url` (`google_video_thumbnail_loc_invalid_url` / `google_video_content_loc_invalid_url` / `google_video_player_loc_invalid_url`) for the populated field, **only** when that field's value is non-empty.
- **Data URL anti-cascade lock (fixed, FIX 4):** a candidate value `data:video/mp4;base64,...` emits `google_video_data_url_unsupported` for the relevant field and **never** also the corresponding `*_invalid_url` solely because the value lacks `://`. Test this explicitly and separately for `thumbnailLoc`, `contentLoc`, and `playerLoc` where applicable.
- **no `FILTER_VALIDATE_URL` behavior may be used as the expected contract** in any Video URL-shape test; the expected acceptance/rejection set must be asserted literally from the exact predicate.
...
- every rejected/missing case (null, whitespace/empty, no-scheme/relative, empty authority, raw space, malformed percent escape) emits **only** the single code `hreflang_url_not_fully_qualified` with exactly one diagnostic per link (no separate missing/invalid/relative codes).
- **no `FILTER_VALIDATE_URL` behavior may be used as the expected contract** in any hreflang URL test; the expected acceptance/rejection set must be asserted literally from the exact fully-qualified predicate.
...
- structurally invalid links (tag-syntax and/or URL code emitted through the **exact** fixed predicates) are excluded from cluster-edge computation: no reciprocity/self-reference/alternate-set cluster diagnostic is derived from them, and no synthetic cascade cluster diagnostic is added solely because they were excluded; no exclusion condition beyond the two exact predicates is tested or implemented.
...
- **Exact lexical predicates are the trigger (FIX 2/3/4/5/6):** a diagnostic whose trigger depends on URL/IRI/hreflang lexical validity must use the exact lexical predicate fixed by this Audit. Implementation libraries, PHP built-ins, regex helpers, URI parsers, or internal utilities may be used only as implementation mechanisms; they must not redefine the observable acceptance/rejection set.
