...
- Missing `og:type` and missing `og:url` are **new** diagnostics introduced by the OGP profile. Their contracts are fixed as:
  ...
  - `evidence_state`: `null` (not evidence/context diagnostics); `related_legacy_code`: `null`;
