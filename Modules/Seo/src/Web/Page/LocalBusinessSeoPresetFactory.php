...
final class LocalBusinessSeoPresetFactory
{
    ...
    public static function servicePage(string $title, ?string $description, array $service, array $business, array $options = []): SeoPagePresetOutputDTO
    {
        ...
        if (array_key_exists('serviceType', $service)) { $builder->setServiceType(DomainSeoPresetFactoryHelper::expectString($service['serviceType'], 'service.serviceType')); }
        if (array_key_exists('areaServed', $service)) { $builder->setAreaServed(is_array($service['areaServed']) ? DomainSeoPresetFactoryHelper::associativeArray($service['areaServed'], 'service.areaServed') : DomainSeoPresetFactoryHelper::expectString($service['areaServed'], 'service.areaServed')); }

        return SeoPagePresetFactory::generic($title, $description, DomainSeoPresetFactoryHelper::appendExtraSchema(self::withBusinessSchema($business, $options, null), new JsonLdSchemaDTO($builder->toArray())));
    }
    ...
    public static function contactPage(string $title, ?string $description, array $business, array $contact = [], array $options = []): SeoPagePresetOutputDTO
    {
        ...
        if ($canonical !== null) { $page->setUrl($canonical); }
        if ($description !== null && $description !== '') { $page->setDescription($description); }
        if (array_key_exists('contactType', $contact)) { $page->setContactPoint(DomainSeoPresetFactoryHelper::expectString($contact['contactType'], 'contact.contactType')); }

        return SeoPagePresetFactory::generic($title, $description, DomainSeoPresetFactoryHelper::appendExtraSchema(self::withBusinessSchema($business, $options, $description), new JsonLdSchemaDTO($page->toArray())));
    }
    ...
    private static function withBusinessSchema(array $business, array $options, ?string $description): array
    {
        ...
        if ($description !== null && $description !== '') { $builder->setDescription($description); }
        if (array_key_exists('telephone', $business)) { $builder->setTelephone(DomainSeoPresetFactoryHelper::expectString($business['telephone'], 'business.telephone')); }
        if (array_key_exists('email', $business)) { $builder->setEmail(DomainSeoPresetFactoryHelper::expectString($business['email'], 'business.email')); }
        if (array_key_exists('logo', $business)) { $builder->setLogo(DomainSeoPresetFactoryHelper::expectString($business['logo'], 'business.logo')); }
        if (array_key_exists('priceRange', $business)) { $builder->setPriceRange(DomainSeoPresetFactoryHelper::expectString($business['priceRange'], 'business.priceRange')); }
        if (array_key_exists('address', $business)) { $builder->setAddress(DomainSeoPresetFactoryHelper::associativeArray($business['address'], 'business.address')); }
